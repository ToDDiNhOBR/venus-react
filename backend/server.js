...
const filiaisApi = require('./filiais-api');  // Import filiais API router
const productsRouter = require('./products');  // Import products router
...
// Set up JSON parsing middleware BEFORE mounting routes
app.use(express.json({ limit: '10mb' }));

// Mount filiais API router
app.use('/api/admin/filiais', filiaisApi.router);

// Mount products router
app.use('/api/products', productsRouter);

// Import and use the initializeSheets function from sheets.js
const { initializeSheets, getSheets, setSheets } = require('./sheets');

// Initialize Google Sheets API and start server
initializeSheets()
  .then((sheetsInstance) => {
    // Set the sheets instance for use in this file
    sheets = sheetsInstance;
    
    // Pass sheets instance to filiais-api module
    filiaisApi.setSheets(sheetsInstance);
  ...
  .catch((error) => {
    console.error('Failed to initialize Google Sheets API. Server not started.', error);
  });
